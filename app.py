...
# 2. Logic Functions
def process_data(df):
    # Fix 'Sample' column naming issue
    potential_names = ['Sample', 'sample', 'SAMPLE', 'Sample No', 'Sample No.']
...
if input_mode == "Manual Entry":
    ...
    data = {"Sample_ID": ["User_1"], "pH": [ph], "Ca": [ca], "Mg": [mg], "Na": [na], "K": [k], "HCO3": [hco3], "CO3": [0], "Cl": [cl], "SO4": [so4], "TDS": [tds]}
    ...
    if uploaded_file is not None:
        ...
        sample_choice = st.sidebar.selectbox("Choose Sample:", df['Sample_ID'].tolist())
...
if input_mode == "Batch Analysis (Upload CSV)":
    total = len(df)
    both_pct = (len(df[df['is_drinking_safe'] & df['is_irrigation_safe']]) / total) * 100
    drink_pct = (len(df[df['is_drinking_safe']]) / total) * 100
    irrig_pct = (len(df[df['is_irrigation_safe']]) / total) * 100
    
    st.subheader("📊 Dataset Statistics")
    m1, m2, m3 = st.columns(3)
    m1.metric("Safe for Both", f"{both_pct:.1f}%")
    m2.metric("Safe for Drinking", f"{drink_pct:.1f}%")
    m3.metric("Safe for Irrigation", f"{irrig_pct:.1f}%")
    st.divider()
...
st.subheader(f"📍 Analysis Result: {current_row['Sample_ID']}")
...
tab1, tab2, tab3 = st.tabs(["📊 Results", "📈 Plots", "📑 Dataset"])
...
with tab1:
    ...
    with col1:
        ...
        st.write(f"Status: **{'✅ SAFE' if current_row['is_drinking_safe'] else '🚨 UNSUITABLE'}**")
    with col2:
        st.subheader("🌾 Irrigation Suitability")
        st.metric("SAR Index", f"{current_row['SAR']:.2f}")
        st.write(f"Status: **{'✅ SAFE' if current_row['is_irrigation_safe'] else '🚨 UNSUITABLE'}**")
...
with tab2:
    ...
    if st.button("Generate Selected Plot"):
        with contextlib.redirect_stdout(io.StringIO()):
            # Ensure plotting columns exist internally
